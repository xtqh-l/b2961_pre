...
class City:
    def __init__(self, home_count=40, work_count=20, subway_count=5, image_positions=None):
        ...
        # Weather system
        self.weather_types = ['clear', 'rain', 'snow', 'heatwave']
        self.current_weather = 'clear'
        self.weather_change_tick = 0
        self._build_city()
    ...
    def _initialize_weather(self):
        ...
        # Set initial weather change tick
        self.weather_change_tick = random.randint(12, 36)  # Change every 1-3 hours (12-36 ticks)
    
    def update_weather(self):
        """Update weather with dynamic changes"""
        self.weather_change_tick -= 1
        if self.weather_change_tick <= 0:
            # Change weather
            old_weather = self.current_weather
            self.current_weather = random.choice(self.weather_types)
            
            # Increase roadworks when entering heatwave
            if self.current_weather == 'heatwave' and old_weather != 'heatwave':
                self._increase_roadworks_during_heatwave()
            
            # Set next change time
            self.weather_change_tick = random.randint(12, 36)  # Change every 1-3 hours
    
    def _increase_roadworks_during_heatwave(self):
        """Increase roadworks during heatwave"""
        ...
        # Calculate how many new roadworks to add (25% more)
        current_roadworks = len(self.roadworks)
        max_possible = len(non_subway_edges)
        additional_roadworks = max(1, int(max_possible * 0.25)) - current_roadworks
        
        if additional_roadworks > 0:
            # Get edges not already in roadworks
            available_edges = [edge for edge in non_subway_edges 
                             if tuple(sorted(edge)) not in self.roadworks]
            
            if available_edges:
                # Add random edges as roadworks
                new_roadworks_count = min(additional_roadworks, len(available_edges))
                new_roadworks = random.sample(available_edges, new_roadworks_count)
                for edge in new_roadworks:
                    self.roadworks.add(tuple(sorted(edge)))
                
                print(f"[DEBUG] Heatwave: Added {new_roadworks_count} additional roadworks")
    ...
    def is_raining(self):
        ...
        return self.current_weather == 'rain'
    
    def is_snowing(self):
        """Check if it's snowing"""
        return self.current_weather == 'snow'
    
    def is_heatwave(self):
        """Check if there's a heatwave"""
        return self.current_weather == 'heatwave'
    ...
    def get_path_time(self, path):
        ...
        for i in range(len(path) - 1):
            ...
            edge_weight = edge_data.get('weight', 5)
            # Double time when snowing
            if self.is_snowing():
                edge_weight *= 2
