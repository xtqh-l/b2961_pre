...
class Person:
    ...
    def step(self, current_time):
        ...
        if self.current_path and self.current_index < len(self.current_path) - 1:
            ...
            # Check if it's raining (increase congestion effects)
            is_raining = getattr(self.city, 'is_raining', lambda: False)()
            
            # Apply congestion delay - lower threshold when raining
            congestion_threshold = 3 if is_raining else 5
            if traffic_count > congestion_threshold:
                # Longer delay when raining
                self.congestion_delay = 2 if is_raining else 1  # Wait extra ticks due to congestion
                return
